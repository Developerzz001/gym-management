...
import { useCreateDieticianMutation, useUpdateDieticianMutation, type DieticianRequest } from '@/api/dieticiansApi';
import { extractErrorMessage } from '@/api/axiosClient';

interface DieticianFormDialogProps {
  open: boolean;
  onClose: () => void;
  dietician?: DieticianResponse | null;
}

export function DieticianFormDialog({ open, onClose, dietician }: DieticianFormDialogProps) {
  const isEdit = !!dietician;
  const [errorMessage, setErrorMessage] = useState<string | null>(null);
  const createMutation = useCreateDieticianMutation();
  const updateMutation = useUpdateDieticianMutation();
  ...
  const formik = useFormik<DieticianRequest>({
    initialValues: {
    ...
    onSubmit: async (values) => {
      ...
      try {
        if (isEdit && dietician) {
          await updateMutation.mutateAsync({ id: dietician.id, payload: values });
        } else {
  ...
  useEffect(() => {
    if (dietician) {
      formik.setValues({
        firstName: dietician.firstName, lastName: dietician.lastName, email: dietician.email,
        mobileNumber: dietician.mobileNumber ?? '', password: '',
        specialization: dietician.specialization ?? '', experienceYears: dietician.experienceYears,
        bio: dietician.bio ?? '', active: dietician.active,
      });
    } else {
      formik.resetForm();
    }
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      <DialogTitle>{isEdit ? 'Update Dietician' : 'Add Dietician'}</DialogTitle>
      <form onSubmit={formik.handleSubmit}>
        ...
        <DialogActions sx={{ p: 2 }}>
          ...
          <Button type="submit" variant="contained">{isEdit ? 'Update' : 'Add Dietician'}</Button>
        </DialogActions>
      </form>
    </Dialog>
  );
